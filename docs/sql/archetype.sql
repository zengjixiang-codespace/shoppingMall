...
create database if not exists `archeType` default character set utf8mb4 collate utf8mb4_0900_ai_ci;
use `archeType`;
...
(
    ...
    primary key (`id`),
    unique key `idx_employee_number` (`employee_number`) # 唯一索引 索引名称 该索引的列
) ENGINE = InnoDB
  AUTO_INCREMENT = 40
...
(
    ...
    `update_time`         DATETIME         NOT NULL COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY `id_employee_number` (`employee_number`)
) ENGINE = InnoDB
  AUTO_INCREMENT = 9
  DEFAULT CHARSET = UTF8;
